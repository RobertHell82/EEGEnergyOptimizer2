...
def test_waerme_zaehlt_unabhaengig_von_der_puffertemperatur():
    """Zwei Takte je 60 s mit 3 kW in den Heizstab. Beide zählen voll zum
    Wärmewert — wie warm der Puffer dabei ist, spielt keine Rolle mehr."""
    b = _bilanz()
    now = datetime(2026, 8, 27, 12, 5, tzinfo=timezone.utc)
    for _ in range(2):
    ...
    assert "heizstab_ueber" not in slot
